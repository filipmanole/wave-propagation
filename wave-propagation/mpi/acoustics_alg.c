...
#pragma omp parallel for private(i, j, place)
    for (i = start; i < stop; i++)
        for (j = 0; j < nx; j++)
        {
            if (!on_corner(rank, numtask, i, j) && !on_edge(rank, numtask, i, j) && !is_source(i, j, radius, source_active) && !on_structure_edge(i, j) && !on_structure_corner(i, j) && !in_structure(i, j))
